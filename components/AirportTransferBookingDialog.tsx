...
function TimePickerField({ label, value, onChange, attempted }: { label: string; value: TimeValue | null; onChange: (time: TimeValue) => void; attempted?: boolean }) {
  ...
  // Sync draft when value is set externally
...
function LocationScheduleFields({ booking, updateBooking, pickupLabel, pickupPlaceholder, destinationLabel, destinationPlaceholder, attempted, citiesOnly, riyadhOnly, pickupRiyadhOnly, supportedCitiesOnly, destinationCity }: { booking: BookingState; updateBooking: (updates: Partial<BookingState>) => void; pickupLabel?: string; pickupPlaceholder?: string; destinationLabel?: string; destinationPlaceholder?: string; attempted: boolean; citiesOnly?: boolean; riyadhOnly?: boolean; pickupRiyadhOnly?: boolean; supportedCitiesOnly?: boolean; destinationCity?: string }) {
  ...
  const riyadhFilter = riyadhOnly
    ...
    : undefined
  // pickupRiyadhOnly: pickup is Riyadh-restricted but destination uses the default (citiesOnly) types
  const pickupLocationRestriction = pickupRiyadhOnly ? RIYADH_RESTRICTION : locationRestriction
  const pickupFilter = pickupRiyadhOnly
    ? (p: { description: string }) => /riyadh/i.test(p.description)
    : riyadhFilter
  ...
  return (
    <div className={styles.fieldGrid}>
      ...
      <PlacesAutocompleteField label={destinationLabel ?? copy.destination} placeholder={destinationPlaceholder ?? copy.selectDropOff} value={booking.destination} attempted={attempted} onChange={value => updateBooking({ destination: value })} types={supportedCitiesOnly ? undefined : cityTypes} locationRestriction={locationRestriction} filterPrediction={destinationFilter} />
...
// IATA flight number: 2-letter airline code + 1–4 digits + optional suffix letter (e.g. EK512, PK753, BA2490)
const FLIGHT_NUMBER_RE = /^[A-Z]{2}\d{1,4}[A-Z]?$/i
function validateFlightNumber(v: string): string | null {
...
function TripDetails({ booking, updateBooking, next, back }: {
  ...
  const tripComplete = Boolean(
    tripIsComplete(booking) &&
    FLIGHT_NUMBER_RE.test(booking.flightNumber.trim().replace(/[\s-]/g, ''))
  )
      ...
      <div className={styles.fieldGrid}>
        ...
        <div style={{ gridColumn: '1 / -1' }}>
          <TextField
            ...
            placeholder={copy.flightExample}
            value={booking.flightNumber}
            minLength={2}
            attempted={attempted}
            validate={validateFlightNumber}
            onChange={flightNumber => updateBooking({ flightNumber })}
            startIcon={<Image src={flightNumberSvg} alt="" width={20} height={18} />}
          />
        </div>
