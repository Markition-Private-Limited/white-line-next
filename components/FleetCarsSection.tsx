...
import img4 from '../assets/fleet/fleet_cars/BMW 5 Series.jpg'
...
export default function FleetCarsSection() {
  ...
  useEffect(() => {
    ...
    const filterObs = new IntersectionObserver(
      ([entry]) => {
        if (entry.isIntersecting) setFilterScrolledPast(false)
        else setFilterScrolledPast(entry.boundingClientRect.top < 0)
      },
  ...
  const FilterButton = ({ idx, fromFloating = false }: { idx: number; fromFloating?: boolean }) => {
    ...
    if (isActive) {
      return (
        <button
          onClick={() => handleFilterClick(idx, fromFloating)}
          style={{
            fontFamily: 'Inter, sans-serif',
            fontSize: fromFloating ? 13 : 'clamp(12px, 1vw, 13px)',
            fontWeight: 600,
            padding: fromFloating ? '7px 16px' : '8px 18px',
            borderRadius: 999,
            border: 'none',
            background: '#005C66',
            color: '#fff',
    ...
    return (
      ...
      >
        <span className="inline-flex items-center transition duration-500 group-hover:-translate-y-[150%]">{label}</span>
        <span className="absolute inset-0 inline-flex items-center justify-center translate-y-full transition duration-500 group-hover:translate-y-0">
          ...
          <span className="relative z-10 font-semibold text-white" style={{ fontSize: fromFloating ? 13 : 'clamp(12px, 1vw, 13px)', fontFamily: 'Inter, sans-serif' }}>
            {label}
          </span>
        </span>
      </button>
