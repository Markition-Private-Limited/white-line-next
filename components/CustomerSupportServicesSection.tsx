...
const CARDS = [
  {
    icon: _src(headphones),
    title: '24/7 Phone Support',
    desc: 'Native speakers prepared to manage high-call volumes, emergency calls and trip tracking around the clock.',
  },
  {
    icon: _src(booking),
    title: 'Booking Assistance',
    desc: 'Welcome changes to active reservations, getting users matched to options directly and effortlessly.',
  },
  {
    icon: _src(cancellation),
    title: 'Cancelation Support',
    desc: 'Smooth handling of incoming cancelation requests with prompt communication to keep drivers updated.',
  },
  {
    icon: _src(rideIssues),
    title: 'Ride Issues',
    desc: 'Solving urgent road situations and matching backup options quickly to maintain dispatcher performance.',
  },
]
  ...
  return (
    <section className="w-full bg-white" style={{ padding: 'clamp(72px, 8vw, 112px) 0' }}>
      <div className="px-6 sm:px-10 lg:px-16">
        ...
        >
          <span style={{ fontWeight: 300 }}>More Than A Ride.</span>
          <br />
          <span style={{ fontWeight: 300 }}>A </span>
          <span style={{ fontWeight: 800, fontStyle: 'italic' }}>Better Experience.</span>
        </motion.h2>
        ...
        >
          Every journey is different. What remains consistent is the experience our passengers
          receive — professional chauffeurs, dependable service, premium vehicles and the comfort
          of knowing everything is taken care of.
        </motion.p>

        {/* Three-column grid: cards | image | cards */}
        <div className="flex flex-col lg:flex-row gap-5 lg:gap-6 items-stretch">

          {/* Left cards */}
          <div className="flex flex-col gap-5 flex-1">
            <ServiceCard {...CARDS[0]} delay={0.1} />
            <ServiceCard {...CARDS[1]} delay={0.2} />
          </div>

          {/* Centre image with parallax */}
          <motion.div
            ref={imgRef}
            className="w-full lg:w-[38%] flex-shrink-0 overflow-hidden"
            style={{
          ...
          {/* Right cards */}
          <div className="flex flex-col gap-5 flex-1">
            <ServiceCard {...CARDS[2]} delay={0.1} />
            <ServiceCard {...CARDS[3]} delay={0.2} />
          </div>
