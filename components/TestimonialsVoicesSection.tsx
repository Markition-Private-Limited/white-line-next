...
function VoiceCard({ voice, scrollYProgress }: { voice: typeof VOICES[0]; scrollYProgress: MotionValue<number> }) {
  ...
  return (
    <div className="relative flex-shrink-0 overflow-hidden" style={{ borderRadius: 'clamp(14px, 1.5vw, 20px)', aspectRatio: '3 / 4' }}>
      <motion.img src={voice.img} alt={voice.name} style={{ y, scale: 1.18, zIndex: 0, position: 'absolute', inset: 0, width: '100%', height: '100%', objectFit: 'cover', objectPosition: 'center' }} className="absolute inset-0 w-full h-full object-cover object-center" />
      <div className="absolute inset-0" style={{ background: 'linear-gradient(180deg, rgba(255,255,255,0) 11.34%, rgba(255,255,255,0) 62%, rgba(255,255,255,0.82) 100%)', zIndex: 1 }} />
      <div className="absolute inset-0 flex items-center justify-center" style={{ zIndex: 2 }}>
        <button aria-label="Play testimonial" className="flex items-center justify-center rounded-full bg-white/90 transition-transform hover:scale-110 active:scale-95"
      ...
      <div className="absolute bottom-0 left-0 right-0 px-5 pb-5" style={{ zIndex: 2 }}>
        <p style={{ fontFamily: 'Montserrat, sans-serif', fontWeight: 700, fontSize: 'clamp(14px, 1.4vw, 18px)', color: '#111118', margin: 0, lineHeight: 1.2 }}>{voice.name}</p>
        <p style={{ fontFamily: 'Inter, sans-serif', fontWeight: 400, fontSize: 'clamp(11px, 1vw, 13px)', color: '#6b7280', margin: '4px 0 0' }}>{voice.role}</p>
      </div>
    </div>
  )
}
...
export default function TestimonialsVoicesSection() {
  ...
  const { voices } = trans.testimonialsPage
  ...
  return (
    <section ref={sectionRef} className="w-full bg-white" style={{ padding: 'clamp(72px, 8vw, 112px) 0' }}>
      <div className="px-6 sm:px-10 lg:px-16">
        ...
        <motion.h2 {...fadeUp(0.08)} className="text-center text-[#111118] leading-tight mb-5" style={{ fontFamily: 'Montserrat, sans-serif', fontSize: 'clamp(26px, 4vw, 52px)' }}>
          <span style={{ fontWeight: 300 }}>{voices.h1}</span>
          <br />
          <span style={{ fontWeight: 800, fontStyle: 'italic' }}>{voices.h2}</span>
        </motion.h2>
        ...
        <motion.div {...fadeUp(0.22)} className="relative">
          <button onClick={prev} disabled={index === 0} aria-label="Previous"
            ...
            style={{ width: 47.5, height: 50, borderRadius: 33, border: '1px solid #C7C7C7', background: '#FFFFFF', padding: 14 }}>
          ...
          <div className="overflow-hidden">
            <div className="flex" style={{ gap: GAP, transform: `translateX(calc(-${index} * (100% / ${visible} + ${GAP} / ${visible} * (${visible} - 1) / ${visible})))`, transition: 'transform 0.55s cubic-bezier(0.4, 0, 0.2, 1)' }}>
              {VOICES.map((v, i) => (
                <div key={i} style={{ width: `calc((100% - ${GAP} * ${visible - 1}) / ${visible})`, flexShrink: 0 }}>
          ...
          <button onClick={next} disabled={index >= Math.max(maxIndex, 0)} aria-label="Next"
