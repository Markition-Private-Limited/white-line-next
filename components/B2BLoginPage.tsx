    ...
    <div className="min-h-screen flex flex-col bg-white">
      ...
      <div className="flex flex-col lg:flex-row flex-1">
        ...
        <div className="relative w-full lg:w-1/2 h-[48vh] lg:h-auto flex-shrink-0 overflow-hidden" style={{ minHeight: 'calc(100vh - 64px)' }}>
          <img src={bannerSrc} alt="B2B" className="absolute inset-0 w-full h-full object-cover object-center" />
          ...
          <div className="absolute top-0 left-0 pt-6 lg:pt-8 px-6 lg:px-10 z-10">
            <div className="flex items-center gap-3">
              <div className="flex flex-col items-center gap-0.5">
                <img src={logoSrc} alt="White Line" style={{ width: 28, height: 30, filter: 'brightness(0) invert(1)' }} />
                <span style={{ fontFamily: 'Montserrat, sans-serif', fontSize: 7, fontWeight: 600, letterSpacing: '0.18em', color: 'rgba(255,255,255,0.55)', textTransform: 'uppercase' }}>
                  WHITE LINE
                </span>
              </div>
              <h1 style={{ fontFamily: 'Montserrat, sans-serif', fontSize: 'clamp(16px, 2vw, 22px)', fontWeight: 600, color: '#fff', letterSpacing: '0.01em' }}>
                {t.portal}
              </h1>
            </div>
          </div>
          ...
          <div className="absolute bottom-14 lg:bottom-16 left-0 right-0 px-6 lg:px-10 z-10 flex flex-col items-center text-center">
            <p style={{ fontFamily: 'Montserrat, sans-serif', fontSize: 'clamp(22px, 3.2vw, 36px)', fontWeight: 700, fontStyle: 'italic', color: '#fff', marginBottom: 8, lineHeight: 1.2 }}>
              {t.tagline}
            </p>
            <p style={{ fontFamily: 'Inter, sans-serif', fontSize: 'clamp(12px, 1.1vw, 14px)', color: 'rgba(255,255,255,0.6)', maxWidth: 560, lineHeight: 1.6 }}>
              {t.sub}
            </p>
          </div>

          {/* Footer strip */}
          <div className="absolute bottom-0 left-0 right-0 px-6 lg:px-10 py-3 flex items-center justify-between z-10" style={{ borderTop: '1px solid rgba(255,255,255,0.08)' }}>
            ...
            <div className="hidden sm:flex items-center gap-4">
              <span style={{ fontFamily: 'Inter, sans-serif', fontSize: 10, color: 'rgba(255,255,255,0.35)', cursor: 'pointer' }}>{t.privacy}</span>
              <span style={{ color: 'rgba(255,255,255,0.15)', fontSize: 10 }}>|</span>
              <span style={{ fontFamily: 'Inter, sans-serif', fontSize: 10, color: 'rgba(255,255,255,0.35)', cursor: 'pointer' }}>{t.terms}</span>
            </div>
          </div>
        </div>
        ...
        <div className="w-full lg:w-1/2 bg-white flex flex-col items-center justify-center px-6 py-10 lg:py-0 relative">
          ...
          <div className="hidden lg:flex absolute inset-0 items-center justify-center pointer-events-none select-none" style={{ zIndex: 0 }}>
            <img src={logoSrc} alt="" style={{ width: 220, height: 240, opacity: 0.05, filter: 'grayscale(1)' }} />
          </div>

          {/* Card */}
          <div className="relative w-full z-10" style={{ maxWidth: 440, background: '#fff', borderRadius: 20, boxShadow: '0 8px 40px rgba(0,0,0,0.10)', padding: 'clamp(24px, 4vw, 40px)' }}>
              ...
              {(['login', 'signup'] as const).map((tabKey) => (
                <button
                  ...
                  style={{
                    ...
                    color: tab === tabKey ? '#005C66' : '#9ca3af',
                    background: 'none',
                    border: 'none',
                    borderBottom: tab === tabKey ? '2px solid #005C66' : '2px solid transparent',
                    marginBottom: -1,
                    cursor: 'pointer',
                    transition: 'all 0.2s',
                  }}
                >
                  {tabKey === 'login' ? t.loginTab : t.signupTab}
                </button>
              ))}
            </div>
            ...
            <div style={{ marginBottom: 18 }}>
              ...
              <div className="flex items-center gap-3" style={{ background: '#f3f4f6', borderRadius: 10, padding: '0 14px', height: 48, border: '1px solid transparent' }}>
            ...
            <div style={{ marginBottom: 20 }}>
              <div className="flex items-center justify-between" style={{ marginBottom: 8 }}>
                ...
                <button style={{ fontFamily: 'Inter, sans-serif', fontSize: 13, fontWeight: 500, color: '#005C66', background: 'none', border: 'none', cursor: 'pointer', padding: 0 }}>
                  {t.forgotPassword}
                </button>
              </div>
              <div className="flex items-center gap-3" style={{ background: '#f3f4f6', borderRadius: 10, padding: '0 14px', height: 48, border: '1px solid transparent' }}>
            ...
            {/* Remember me */}
            <label className="flex items-center gap-2.5 cursor-pointer select-none" style={{ marginBottom: 28 }} onClick={() => setRemember(v => !v)}>
              <div style={{ width: 18, height: 18, borderRadius: '50%', border: `1.5px solid ${remember ? '#005C66' : '#d1d5db'}`, background: remember ? '#005C66' : 'transparent', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0, transition: 'all 0.2s' }}>
                {remember && (
              ...
              <span style={{ fontFamily: 'Inter, sans-serif', fontSize: 14, color: '#6b7280' }}>{t.rememberMe}</span>
            </label>
